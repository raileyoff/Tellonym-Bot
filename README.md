# 🌐 Tellonym Bot – Anonymous Messages & Secrets

> Send and receive **anonymous messages, secrets, and feedback** directly on Discord — safely, privately, and with style.  
> Perfect for friends, communities, and creators who want honest and fun interactions!

---

## ✨ Features

- 💬 Send & receive anonymous messages  
- 🔒 100% privacy and security focused  
- ⚙️ Simple and intuitive commands  
- 🎨 Clean, modern, and customizable design  
- 🚫 Anti-spam and abuse protection  
- 🌍 Works on any Discord server  

---

## 💡 About the Bot

**Tellonym Bot** brings the Tellonym experience to Discord —  
a fun and secure way to connect, share opinions, confessions, or feedback without revealing your identity.  

It’s built to make communication more **honest, open, and enjoyable** for everyone.  

---

## 🌍 Multilingual Description

### 🇬🇧 English
Send and receive anonymous messages directly on Discord!  
Let your friends share honest thoughts or secrets safely and privately with a modern Tellonym-style bot.

### 🇫🇷 Français
Envoyez et recevez des messages anonymes directement sur Discord !  
Laissez vos amis partager leurs avis, secrets ou confidences en toute sécurité grâce à un bot moderne inspiré de Tellonym.

### 🇸🇦 العربية
أرسل واستقبل **رسائل مجهولة الهوية** مباشرة على **ديسكورد**!  
دع أصدقاءك يشاركونك آرائهم أو أسرارهم بطريقة آمنة وخاصة مع بوت مستوحى من **Tellonym**.

---

## 📢 Invite the Bot

👉 **[Invite Tellonym Bot to your server](https://discord.com/oauth2/authorize?client_id=1387642831497334934)**  

---

## 🧠 Developed by

**NextGen Team**  
⚡ Discord Bot Developers | Custom Solutions | Innovation for Communities  
Developed by **@raileyoff** — one of the main developers of the NextGen team.

---

### 📜 License
This project is protected under the **MIT License**.  
You’re free to use, modify, and share it — just credit the NextGen team.

---

> 💌 *Tellonym Bot – Where honesty meets fun.*

